...
unless defined?(AwesomePrint)
  ...
  require File.dirname(__FILE__) + "/awesome_print/ext/active_record"  if defined?(ActiveRecord::Base) || (defined?(IRB) && ENV['RAILS_ENV'])
  ...
  require File.dirname(__FILE__) + "/awesome_print/ext/mongo_mapper"   if defined?(MongoMapper::Document)
  require File.dirname(__FILE__) + "/awesome_print/ext/mongoid"        if defined?(Mongoid::Document)
